...
template<typename A>
auto operator*(int s, A&& a) -> typename std::enable_if<std::is_base_of_v<BaseVectorOps, std::remove_reference_t<A>>, VectorMulByScalarOp<A>>::type {
  return VectorMulByScalarOp<A>(s, std::forward<A>(a));
}

template<typename A>
auto operator*(A&& a, int s) -> typename std::enable_if<std::is_base_of_v<BaseVectorOps, std::remove_reference_t<A>>, VectorMulByScalarOp<A>>::type {
  return VectorMulByScalarOp<A>(s, std::forward<A>(a));
}

template<typename A, typename B>
auto operator+(A&& a, B&& b) -> typename std::enable_if<std::is_base_of_v<BaseVectorOps, std::remove_reference_t<A>>, VectorAddOp<A, B>>::type {
  return VectorAddOp<A, B>(std::forward<A>(a), std::forward<B>(b));
}

template<typename A, typename B>
auto operator-(A&& a, B&& b) -> typename std::enable_if<std::is_base_of_v<BaseVectorOps, std::remove_reference_t<A>>, VectorAddOp<A, VectorMulByScalarOp<B>>>::type {
...
auto operator+(D& a, E& b) -> typename std::enable_if<std::is_base_of_v<Base, D>, A>::type  {
  A c{a.a+b.a};
  return c;
}
